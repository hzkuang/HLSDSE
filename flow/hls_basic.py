...
import os
import json
from common import createFolder, getYaml, basicOption


class HLSBasic(object):
    def __init__(self, root, mode, bench, case, ver, encode, num=100,
                 alg='motpe', device='xc7vx485tffg1761-2', clk='10'):
        self.root = root
        self.mode = mode
        self.bench = bench
        self.case = case
    ...
    def get_dataset_path(self):
        if self.encode == 'discrete':
            self.dataset_path = os.path.join(self.root, 'dse_ds', self.bench, self.encode + '_ds', self.case)
        else:
    ...
    def gen_hls_temp_script(self):
        ...
        fw.write('open_project {}_{}_prj\n'.format(self.case, self.alg))
    ...
    def config_space(self):
        ...
        for func in funcList:
            if func is None:
                ...
                paraFunc.update({('F_' + str(cntF)): 0.0})
                cntF = cntF + 1
